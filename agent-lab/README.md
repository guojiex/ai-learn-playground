...
本目录的设计文档已全部完成，代码按里程碑逐步落地 (M0–M11 全部完成)。
...
在仓库根目录执行一条命令启动本地大模型 + 网页：

```powershell
.\agent-lab\tools\run.ps1 local-web
```

启动成功后打开 `http://127.0.0.1:8090/`。

脚本会自动创建并复用 `agent-lab/.venv` 虚拟环境，依赖来自 `scripts/python-openai-server/requirements.txt`，不会安装到全局 Python 环境。
...
换到真实本地模型时也可以把 `OPENAI_BASE_URL` 指向 `llama-server` / `ollama`，详见 [docs/04-local-model-stack.md](docs/04-local-model-stack.md)。
